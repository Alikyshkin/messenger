...
**ВАЖНО:** Перед каждым коммитом автоматически проверяется:
...
При первом клонировании репозитория или после обновления hooks запустите:

```bash
./scripts/install-hooks.sh
```

Это установит git hook, который будет автоматически проверять код перед каждым коммитом.
...
При пуше в `main` или `develop` автоматически запускается GitHub Actions workflow (`.github/workflows/ci.yml`), который проверяет:
- Форматирование Flutter кода
- Анализ Flutter кода
- Полная сборка для Web (если не проходит — CI упадёт)
- Проверка Node.js сервера (если настроены скрипты lint/type-check)
