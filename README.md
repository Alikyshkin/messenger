...
- ⚙️ **[Настройка секретов](docs/setup-secrets.md)** — настройка GitHub Actions, SSH
- 🚢 **[Развёртывание](docs/deployment.md)** — полное руководство по развёртыванию на VPS, HTTPS
...
1. **VPS (сервер в облаке)**
   Арендуете виртуальный сервер (например **Hetzner**, **DigitalOcean**, **Selectel**, **Timeweb**), ставите Node.js, клонируете репозиторий, настраиваете `.env`, запускаете сервер.
...
**Рекомендация для начала:** VPS с Ubuntu, установить Node.js + PM2, настроить Nginx + Let's Encrypt для HTTPS, в cron раз в день копировать `messenger.db` в бэкап.
