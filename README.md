...
**ВАЖНО:** Проект использует множество автоматических проверок для обеспечения качества кода.

### Локальная проверка (pre-commit hook)

Перед каждым коммитом автоматически проверяется:
- Форматирование кода (`flutter format`)
- Анализ кода (`flutter analyze`)
- Компиляция для Web (`flutter build web`)
...
После установки hook при коммите автоматически запускается проверка. Если есть ошибки — коммит будет отклонён.

Чтобы пропустить проверку (не рекомендуется):
```bash
...
При пуше в `main` или `develop` автоматически запускаются проверки:

**Основные проверки** (`.github/workflows/ci.yml`):
- Форматирование Flutter кода
- Анализ Flutter кода
- Полная сборка для Web
- Проверка Node.js сервера

**Безопасность** (`.github/workflows/security.yml`):
- Проверка зависимостей на уязвимости (npm audit)
- CodeQL анализ кода
- Dependency Review для PR

**Качество кода** (`.github/workflows/code-quality.yml`):
- Покрытие тестами (code coverage)
- Размер бандла (bundle size check)
- Проверка сообщений коммитов

**Производительность** (`.github/workflows/performance.yml`):
- Lighthouse проверка производительности веб-версии

**Если проверки не прошли — код не будет задеплоен.**

### Автоматические обновления зависимостей

Проект использует **Dependabot** для автоматического создания PR с обновлениями зависимостей:
- Flutter/Dart пакеты — еженедельно
- Node.js пакеты — еженедельно
- GitHub Actions — ежемесячно

Все обновления проходят те же проверки CI/CD перед мерджем.
...
**Примеры:**
```
feat(chat): добавлена поддержка реакций на сообщения
fix(auth): исправлена проверка токена при обновлении
docs(readme): обновлена инструкция по установке
chore(deps): обновлен flutter_webrtc до 1.3.0
```

## Дополнительные практики разработки

### Code Review

Все изменения проходят через Pull Request с обязательным ревью:
- Используйте шаблон PR (автоматически подставляется)
- Убедитесь, что все проверки CI прошли
- Дождитесь одобрения перед мерджем

### Мониторинг и логирование

- **Prometheus + Grafana** — метрики производительности сервера
- **Pino** — структурированное логирование на сервере
- Логи доступны через Docker Compose: `docker-compose -f docker-compose.monitoring.yml up`

### Безопасность

- Регулярные проверки уязвимостей в зависимостях
- CodeQL анализ кода на GitHub
- Использование секретов для чувствительных данных
...
Подробнее в разделе "Версионирование приложения" ниже.
